...
const activeTasksCount = document.getElementById('active-tasks-count') // находим элемент для отображения количества активных задач по id
...
taskForm.addEventListener('submit', function(event) {  // слушаем сабмит (отправку формы)
    ...
    deleteButton.addEventListener('click', function() { // слушаем click по кнопке удаления
        ...
        updateActiveCount(); // обновляем счётчик активных задач при удалении задачи
    ...
    completeCheckbox.addEventListener('change', function () { // слушаем изменение состояния чекбокса
        ...
        updateActiveCount(); // обновляем счётчик активных задач при изменении состояния чекбокса
